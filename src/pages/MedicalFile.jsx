...
const MedicalFile = () => {
  ...
  const handleExportPDF = () => {
    ...
    line('HEALYTICS — MEDICAL FILE REPORT');
    ...
    line('Patient ID  : ' + (user?.refID || '—'));
    line('Age         : ' + (patient?.Age || '—'));
    line('Gender      : ' + (patient?.Gender || '—'));
    line('Condition   : ' + (patient?.Condition || '—'));
    blank();

    line('MEDICAL RECORDS (' + records.length + ')');
    line('-'.repeat(40));
    if (records.length === 0) {
      line('No medical records found.');
    } else {
      records.forEach((r, i) => {
        line(`[${i + 1}] Date: ${new Date(r.VisitDate).toLocaleDateString()}  |  Doctor: ${r.DoctorName}`);
        line('    Diagnosis : ' + r.Diagnosis);
        line('    Treatment : ' + r.Treatment);
        if (r.SymptomInput) line('    Symptoms  : ' + r.SymptomInput);
        blank();
      });
    }

    line('ACTIVE PRESCRIPTIONS (' + prescriptions.length + ')');
    line('-'.repeat(40));
    if (prescriptions.length === 0) {
      line('No active prescriptions.');
    } else {
      prescriptions.forEach((p, i) => {
        line(`[${i + 1}] ${p.Medication}  —  ${p.Dosage}, ${p.Frequency}, ${p.DurationDays} days`);
      });
    }
    blank();

    line('APPOINTMENTS (' + appointments.length + ')');
    line('-'.repeat(40));
    if (appointments.length === 0) {
      line('No appointments found.');
    } else {
      appointments.forEach((a, i) => {
        line(`[${i + 1}] ${new Date(a.AppointmentDate).toLocaleDateString()}  |  ${a.DoctorName} (${a.Specialty})  |  ${a.Status}`);
      });
    }

    blank();
    line('='.repeat(60));
    line('⚠ This document is generated by Healytics and is for informational purposes only.');
    line('   Always consult your healthcare provider for medical decisions.');
    ...
    a.href = url;
    a.download = `medical-file-${user?.refID || 'patient'}-${new Date().toISOString().split('T')[0]}.txt`;
    a.click();
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white border-b px-6 py-3 flex items-center gap-3">
          ...
          <button onClick={handleExportPDF} className="bg-blue-600 text-white text-xs px-4 py-2 rounded-lg hover:bg-blue-700">
            ⬇ Export Report
          </button>
        </div>
        ...
        <div className="p-5">
          ...
          <div className="bg-white rounded-xl border p-5 mb-5 flex items-center gap-5">
            <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-xl font-medium flex-shrink-0">
